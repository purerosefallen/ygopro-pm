...
function scard.initial_effect(c)
	...
	pm.evoop(scard.cfilter,PM_LOCATION_IN_PLAY,0,scard.evofilter,LOCATION_DECK,0),pm.nfturncon)
end
scard.pokemon_card=true
function scard.cfilter(c,e,tp)
	if not (c:IsFaceup() and c:IsPokemon() and c:IsCanEvolve()) or c:IsStatus(PM_STATUS_PLAY_TURN) then return false end

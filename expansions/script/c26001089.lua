...
function scard.tbtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,PM_LOCATION_BENCH)>0
		and Duel.IsExistingMatchingCard(scard.tbfilter,tp,PM_LOCATION_DPILE,0,1,nil,e,tp) end
end
function scard.tbop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,scard.tbfilter,tp,PM_LOCATION_DPILE,0,1,1,nil,e,tp)
	if g:GetCount()==0 then return end
	Duel.PutInPlay(g,0,tp,tp,false,false,PM_POS_FACEUP_UPSIDE)
end
